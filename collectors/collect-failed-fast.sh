...
# Fast collector - only gets latest failed PR per component
...
echo "========================================"
echo -e "${BLUE}Recolector Rápido de Fallos${NC}"
echo "========================================"
echo ""
...
docker exec ci-autohealing-db psql -U postgres -d konflux_monitoring -c \
    "INSERT INTO scan_history (scan_id, scan_type, scan_mode, status) VALUES ('$SCAN_ID', 'manual', 'quick', 'running');" > /dev/null
...
while IFS= read -r component; do
    ...
    # Get only recent failed PRs (limit 5 for speed)
    echo -n "  Buscando fallos recientes... "

    FAILED_PRS=$(timeout 20 bash -c "yes '' 2>/dev/null | kubectl tekton get pr -n '$NAMESPACE' \
        --labels='appstudio.openshift.io/component=$component' \
        --limit 5 2>/dev/null" | \
        grep -v "^NAME" | \
        grep -v "^Next" | \
        grep -v "^$" | \
        grep -v "Press any key" | \
        grep "Failed" || echo "")
    ...
    FAILED_COUNT=$(echo "$FAILED_PRS" | wc -l)
    echo -e "${RED}$FAILED_COUNT${NC}"

    # Extract PR names
    PR_NAMES=$(echo "$FAILED_PRS" | awk '{print $1}')
    ...
    while IFS= read -r pr_name; do
        ...
        # Get PR details (try Tekton Results first)
        ...
        if [ -z "$PR_JSON" ]; then
            echo "    ✗ $pr_name (no se pudo obtener detalles)"
            continue
        fi
